...
@section('content')
            ...
            <div class="row row-offcanvas row-offcanvas-right">
                ...
                @include('admin.partials.sidebar')
                    ...
                    <div class="content-wrapper">
                        <h1 style="text-align: center;" class="page-title">{{ __('Update this Hot Deal') }}</h1>
                        <div class="card">
                            <div class="card-body">
                                <form method="POST" action="{{URL::to('/update-hot-deal/'.$hotdeal->id)}}" enctype="multipart/form-data">
                                    @method('patch')
                                    @csrf
                                    
                                    <div class="form-group">
                                        <label for="discount">{{ __('Discount Percentage:') }}</label>
                                        <input id="discount" type="text" class="form-control @error('discount') is-invalid @enderror" name="discount" placeholder="Enter the discount percent" value="{{ $hotdeal->discount }}" autocomplete="discount" autofocus>
                                        @error('discount')
                                    ...
                                    <div class="form-group">
                                        <label for="coverimage">{{ __('Image (Optional):') }}</label>
                                    ...
                                    <div class="form-group">
                                        <label for="date">{{ __('Expire Date (Optional):') }}</label>
                                    ...
                                    <div class="form-group mt-5">
                                        ...
                                        <a class="btn btn-outline-danger" href="{{ route('all.hot.deal') }}">{{ __('Cancel') }}</a>
                                    </div>
                                </form>
                            </div>
                        </div>
    ...
    <script>
        ...
        document.title = title + " - Edit Hot Deal"
    </script>
@endsection
