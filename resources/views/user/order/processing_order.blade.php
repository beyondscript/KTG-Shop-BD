                                                    ...
                                                    @foreach($orderdetail as $ordrdtl)
                                                        @if($ordrdtl->status != 'Delivered' && $ordrdtl->status != 'Cancelled')
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <a class="fa fa-eye" style="color: green; text-decoration: none; padding-right: 5px;" data-toggle="tooltip" data-placement="top" title="View Order" href="{{URL::to('/view-order/'.$ordrdtl->id)}}"></a>
                                                                    <a style="color: blue; text-decoration: none; padding-left: 5px;" class="fa fa-trash" data-toggle="tooltip" data-placement="top" title="Cancel Order" href="{{'/cancel-order/'.$ordrdtl->id}}" onclick="destroy(event)">
                                                                        <form style="display: none;" method="post" action="{{URL::to('/cancel-order/'.$ordrdtl->id)}}">
                                                                            @method('delete')
                                                                            @csrf
                                                                        </form>
                                                                    </a>
